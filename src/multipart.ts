...
export function UploadFile(fieldName: string) {
    return function (target: any, propertyKey: string | symbol, parameterIndex: number) {
        ...
        const existingMetadata = Reflect.getMetadata(REQUEST_BODY_FILE_KEY, target, propertyKey) as {
            fieldName: string;
            index: number;
        }[];
        existingMetadata.push({ fieldName, index: parameterIndex });
        Reflect.defineMetadata(REQUEST_BODY_FILE_KEY, existingMetadata, target, propertyKey);
    };
}
...
export function UploadFiles(fieldName: string) {
    return function (target: any, propertyKey: string | symbol, parameterIndex: number) {
        ...
        const existingMetadata = Reflect.getMetadata(REQUEST_BODY_FILES_KEY, target, propertyKey) as {
            fieldName: string;
            index: number;
        }[];
        existingMetadata.push({ fieldName, index: parameterIndex });
        Reflect.defineMetadata(REQUEST_BODY_FILES_KEY, existingMetadata, target, propertyKey);
    };
}
